...
<link type="text/css" rel="stylesheet" href="res/css/search.css" />
</head>
<body>
	<div class="inner">
		<div class="hd">
		...
		<div class="infos">
			<form method="post" action="findFriend">
				<div id="entry">
					<div class="form ">
						<label for="qq"></label>
						<div class="item-ifo">
			...
			<div class="right">
				...
				<c:if test="${!empty users}">
					<c:forEach items="${users}" var="users" varStatus="status">
						<div class="qquser">
							<img src="res/headpic/${users.headpic}" width="80" height="80" />
							<br>昵称：${users.username} <br>QQ：${users.qq}
							<form action="askforFriend" method="post">
								<input type="hidden" name="friendid" value="${users.userid}">
								<input type="submit" value="添加">
							</form>
						</div>
					</c:forEach>
				</c:if>
				<c:if test="${!empty resultisnul}">
					<div class="qquser">
						<b>${resultisnull} </b>
					</div>
				</c:if>
			...
			<div class="c_list clf">
				<div id="pl_page">
					<div class="pagelistbox">
						<p>
							<c:if test="${!empty users }">
								<span> w<c:if test="${pager.isFirstPage==false}">
										<a
